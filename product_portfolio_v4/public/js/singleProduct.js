...
import { checkWishlistStatus } from "./wishListStatus.js";
import { buyNow } from "./buyNow.js";
import { getProductQuantity } from "./getProductQty.js";
...
const fetchProductReviews = async (productId) => {
    try {
        const response = await fetch(`https://happywaycreations.147.93.106.209.nip.io/getProductReview/${productId}`);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        return await response.json();
