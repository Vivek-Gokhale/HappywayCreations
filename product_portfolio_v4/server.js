...
const insertOrder = (orderData, callback) => {
    const { userEmail, products, totalAmount, status, shippingAddress } = orderData;
    const orderId = generateOrderId();
    
    const query = `INSERT INTO orders (order_id, user_email, total_amount, status, shipping_address, order_date) 
                   VALUES (?, ?, ?, ?, ?, NOW())`;
                   
    db.query(query, [orderId, userEmail, totalAmount, status || 'Pending', shippingAddress], (err, result) => {
        if (err) {
            return callback(err);
        }
        ...
        const itemsQuery = `INSERT INTO order_items (order_id, product_id, quantity, price) 
                           VALUES ?`;
                           
        db.query(itemsQuery, [orderItems], (err, result) => {
            if (err) {
                return callback(err);
            }
...
app.post('/create-order', (req, res) => {
    ...
    if (!orderData.userEmail || !orderData.products || !orderData.totalAmount || !orderData.shippingAddress) {
        return res.status(400).json({ error: 'Missing required order information' });
    }

    insertOrder(orderData, (err, orderId) => {
        if (err) {
...
app.get('/get-orders/:userEmail', (req, res) => {
    const userEmail = req.params.userEmail;
    
    const query = `
        SELECT o.*, oi.product_id, oi.quantity, oi.price, p.name as product_name, p.image_url
        FROM orders o
...
app.post('/get-product-cart-details', (req, res) => {
    ...
    const productQuery = `SELECT product_id, product_mrp, product_discount, tax FROM product WHERE product_id IN (${placeholders})`;
